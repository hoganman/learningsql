...
class Employee(Base):
    ...
    superior_emp_id: Mapped[Optional[int]] = mapped_column(ForeignKey("employee.emp_id"), nullable=True)
    """Superior officer primary key as a foreign key, nullable if no superior"""

    dept_id: Mapped[Optional[int]] = mapped_column(ForeignKey("department.dept_id"), nullable=True)
    ...
    assigned_branch_id: Mapped[Optional[int]] = mapped_column(ForeignKey("branch.branch_id"), nullable=True)
    """Assigned branch ID of the employee as a foreign key, nullable if no assigned branch"""

    superior_emp: Mapped[Optional["Employee"]] = relationship("Employee", remote_side=[emp_id])
    """Adjacency relationship that accesses the superior (Employee) of the employee"""

    employee_dept: Mapped[Optional["Department"]] = relationship("Department", back_populates="dept_employees")
    """Relationship that accesses the assigned department of the employee"""

    employee_branch: Mapped[List["Branch"]] = relationship("Branch", back_populates="branch_employees")
    ...
    def __repr__(self) -> str:
        return (
                ...
                " title=%s, assigned_branch_id=%s)" % (
                    ...
                    str(self.assigned_branch_id) if self.assigned_branch_id is not None else "",
                )
        )
